  ...
  if (Vue.ls.get(ACCESS_TOKEN)) {
    ...
    if (to.path === '/user/login' || to.path === OAUTH2_LOGIN_PAGE_PATH) {
      ...
      if (store.getters.roles.length === 0) {
         ...
         store.dispatch('GetPermissionList').then(res => {                   //获取权限菜单数据---改为--拉取用户信息
              let menus=res.data.menus;
              console.log(menus)
              let username=res.data.username;
              console.log("GetPermissionList同getInfo获取登陆用户信息："+username)
              if (menus === null || menus === "" || menus === undefined) {
                return;
              }
              let constRoutes = [];
              constRoutes = generateIndexRouter(menus);
              // 添加主界面路由
              ...
              //   const redirect = decodeURIComponent(from.query.redirect || to.path)  //对URL进行解码
              //   if (to.path === redirect) {
